...
### PDCA 사이클 4단계 — 설계 → 구현 → 검증 → 배포

11개 세부 단계는 크게 **PDCA 사이클 4단계**로 묶입니다(Plan·Do·Check·Act = 설계·구현·검증·배포).
설계와 구현 사이에는 **API 계약 GATE**(관문)가 있어, 계약이 확정돼야 구현에 들어갑니다.

| PDCA 단계 | # | 세부 단계 | 도구(스킬/에이전트) |
|---|---|---|---|
