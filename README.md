...
* **경로 해석 우선순위(2.16.0 부터 도구 전체가 동일):** 환경 변수 `DW_VAULT_DIR` ➔ 프로젝트에 설치된 `<프로젝트>/.claude/dw-config.json` 의 `vault_root`(상위 폴더·git 본체 저장소까지 찾습니다 — 작업 사본(worktree) 대응) ➔ 기본 규약 경로(`~/denver-workflow-vault`) ➔ 그래도 없으면 에러를 반환하며 서버 구동이 중지됩니다.
  * 앞 순위가 **없는 폴더**를 가리키면 조용히 포기하지 않고 다음 순위를 시도합니다.
  * 값에는 리터럴 `~/`·`$HOME/`·`%USERPROFILE%\` **접두**를 쓸 수 있습니다(그 접두만 확장됩니다 — 경로 중간의 `$변수` 는 그대로 둡니다).
  * **출처가 서로 다른 vault 를 말하면** 세션 시작 안내문과 헬스체크(`/dw-review`)에 경고가 뜹니다. vault 는 하나여야 하고, 조용히 하나를 고르면 두 곳을 가리키는 상태가 오류 없이 지나가기 때문입니다.
