...
### 4. Self-hosted CI runner maintenance wiring (`make wire-ci-runners`)

`make wire-ci-runners M=<VM name>` idempotently wires **two axes** per VM. It **auto-discovers** runners from the VM's systemd units (`actions.runner.*.service`) — no project/service names hard-coded. Supports `DRY=1` (read-only inspect) and `RESTART=1` (restart only runners with no active job). 🔴 **Wire BOTH dw-ci and dw-deploy** (one `M=dw-ci` is not enough).

**(A) job-completed docker-volume reclaim hook** — stops the **anonymous docker-volume leak** left by CI `services:` containers (postgres, redis, …); a runner only `docker rm`s the container (WITHOUT `-v`) at job end.

* **Hook:** `ci-runner/job-completed-prune.sh` — set as the runner's `ACTIONS_RUNNER_HOOK_JOB_COMPLETED`, it runs `docker volume prune -f` after each job (dangling anonymous volumes only; a concurrent job's active service volume is attached to a live container and is safe). `timeout`-guarded; always `exit 0` (non-blocking). Dangling-image reclaim is opt-in via `DW_PRUNE_IMAGES=1`.
* **When it arms:** a runner reads `.env` only at service start, so it takes effect on the **next runner restart**; `RESTART=1` arms idle runners immediately (never kills a running job).

**(B) serve-stale forwarding resolver (unbound)** — mitigates the egress transient where a host-Mac Wi-Fi uplink blip breaks CI checkout. The OrbStack DNS proxy (0.250.250.200) translates upstream unreachability into **NXDOMAIN (EAI_NONAME)**, so checkout's "Set up job / Download action" fails to find codeload (a workflow retry can't help).

* **Drop-in:** `ci-runner/serve-stale-resolver.conf` → `/etc/unbound/unbound.conf.d/zz-dw-ci-resolver.conf`. `serve-expired: yes` (RFC 8767) + negative-cache off + `module-config: "iterator"`. Only `orb.local` goes to the OrbStack proxy; **everything else (`.`) bypasses the proxy straight to public resolvers (1.1.1.1 / 8.8.8.8)**.
* 🔴 **Why bypass the proxy:** serve-expired engages only on upstream *failure* (timeout/SERVFAIL); **NXDOMAIN is a valid answer and passes through** (proven by a local unbound 1.19 sim). Sending a blip to the proxy returns NXDOMAIN and serve-stale never fires; a public resolver fails honestly (timeout) → serve-expired fires → last-good A served → checkout survives.
* **Re-arm:** should OrbStack ever rewrite the read-only resolv.conf symlink at VM boot, a systemd oneshot (`ci-runner/dw-resolv-repoint.service`, `After=unbound.service`) re-points `/etc/resolv.conf` to 127.0.0.1 at boot. 🔴 Measured (dw-ci): the symlink mtime is unchanged across a boot → boot regeneration is not confirmed (a plain-file replacement likely persists) — belt-and-suspenders, to be confirmed by a post-deploy reboot check. A fallback nameserver (0.250.250.200) is kept, so if unbound dies DNS degrades to the proxy rather than breaking.
* **Safe ordering:** install drop-in → proceed **only if `unbound-checkconf` passes** → repoint resolv.conf **only if the unbound health check passes** (active + :53 listen). dry-run is fully read-only.
* **Verification limit:** serve-stale engages only *during* an upstream blip, so at wiring time (upstream healthy) it can't be reproduced live — real efficacy is confirmed on the next blip after deploy.
