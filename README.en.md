...
* **Re-arm:** OrbStack regenerates the read-only resolv.conf symlink on every VM boot, so a systemd oneshot (`ci-runner/dw-resolv-repoint.service`, `After=unbound.service`) re-points `/etc/resolv.conf` to 127.0.0.1 at boot. A fallback nameserver (0.250.250.200) is kept, so if unbound dies DNS degrades to the proxy rather than breaking.
