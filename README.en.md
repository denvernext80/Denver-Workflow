...
* **Wiring:** `make wire-ci-runners M=<VM name>` (= `dw.py wire-ci-runners --machine`). **Auto-discovers** runners from the VM's systemd units (`actions.runner.*.service`) — no project/service names hard-coded — copies the hook to `$HOME/.dw-runner-hooks/` (755), and **idempotently** upserts `ACTIONS_RUNNER_HOOK_JOB_COMPLETED` into each runner's `.env`. Supports `DRY=1` (inspect only) and `RESTART=1` (restart only runners with no active job).
