...
### 4-1. API 명세 최초 1회 만들기

설치가 끝나면 **딱 한 번** API 명세(이 프로젝트가 제공하는 모든 API 를 적어 둔 문서)를 만든다.
이후 모든 API 작업이 이 문서를 먼저 읽고 시작한다.

1. `dw_search("API 명세 전체 인덱스")` 로 이미 있는지 확인한다.
   - **이미 있으면** — 아무것도 하지 않는다. 사용자에게 "API 명세가 이미 있습니다(N개 도메인).
     다시 훑으려면 `/dw-api-spec 재추출` 을 쓰세요" 라고만 알린다. **덮어쓰지 않는다.**
   - **없으면** — 2번으로.
2. 사용자에게 확인받는다: "이 프로젝트의 API 를 전부 훑어서 명세서를 만들까요? (기존 프로젝트면
   시간이 좀 걸립니다)" 동의하면 **api-spec-bootstrap 절차**를 수행한다.
3. API 가 없는 프로젝트(프론트 전용·라이브러리·빈 신규 프로젝트)여도 **빈 명세를 만든다** —
   절차의 케이스 B. "명세가 없다" 와 "명세가 비어 있다" 는 다르다. 후자여야 첫 API 를 만들 때
   갱신할 대상이 있다.

멀티레포면 레포마다 따로 만든다(한 인덱스에 두 시스템을 섞지 않는다).

### 4-2. 배치·크론 명세 최초 1회 만들기

같은 방식으로 **딱 한 번** 정기 실행 작업(배치=모아서 한 번에 처리, 크론=정해진 시각에 자동 실행)
명세를 만든다.

1. `dw_search("배치 크론 명세 전체 인덱스")` 로 이미 있는지 확인한다.
   - **이미 있으면** — 아무것도 하지 않고 "다시 훑으려면 `/dw-batch-spec 재추출`" 만 알린다.
   - **없으면** — 2번으로.
2. 사용자에게 확인받는다: "이 프로젝트의 정기 실행 작업(배치·크론)을 전부 찾아 명세서를 만들까요?"
   동의하면 **batch-cron-spec-bootstrap 절차**를 수행한다.
3. 정기 실행이 하나도 없어도 **빈 명세를 만든다** — 절차의 6번.
4. **호스트에 설치된 크론은 운영 접근이 있어야 확인된다.** 접근이 없으면 그게 정상이니
   `미확인 (최종 확인 YYYY-MM-DD, 사유)` 로 남기고 넘어간다. **접근을 얻으려고 방화벽·보안 설정을
   바꾸지 않는다.** 나중에 접근 가능할 때 `/dw-batch-spec 재추출` 로 채운다.
