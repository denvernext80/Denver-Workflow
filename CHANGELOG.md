...
- **SessionStart 다이제스트: 484줄/55,288자 → 257줄/18,537자** (실측, balipick scope union)
  ...
  - `dw-session-context.py` 의 `MAX_BYTES`(60,000자) 대비 **92% → 31%** — 무성 잘림 위험 해소.
...
- **인덱스 요지(`_gist`)를 어절/문장 경계에서 절단** + 말줄임. 인덱스는 모델이 "어느 파일을
  펼칠지" 고르는 라우팅 표면이라 단어 중간 절단이 판단을 방해한다.
