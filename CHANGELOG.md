...
`dw-selftest.py` 에 `VerifierScopeFailOpenTest` 10건. 변이 7종으로 가드가 공허하지 않음을 실증했다
(극성 되돌림 → 7건 RED, staged·untracked 제거 → 2건, 뒤처짐 감지 제거 → 1건, 출력 계약 파괴 → 1건,
base rc 확인 제거 → 1건). ⚠️ `test_docs_only_still_skips` 는 **퇴화 가드**다 — 이게 없으면
「무조건 전량 디스패치」로 퇴화한 구현도 나머지를 전부 통과한다.
