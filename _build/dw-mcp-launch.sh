#!/bin/sh
# dw-vault MCP 자가 부트스트랩 런처 (플러그인 설치 환경용).
...
# vault 콘텐츠 위치 해석 순서: DW_VAULT_DIR(env) > ~/denver-workflow-vault(규약 경로) > 에러.
#   - 기본(규약): ~/denver-workflow-vault. 'make scaffold-vault' 로 생성. 별도 env 설정 불요.
#   - 커스텀 위치: DW_VAULT_DIR 를 원하는 절대경로로 설정. 공백 포함 경로 안전(따옴표).
#     예: export DW_VAULT_DIR="$HOME/My Vaults/denver"
...
if [ -n "$DW_VAULT_DIR" ] && [ -d "$DW_VAULT_DIR" ]; then
  ...
  [ -n "$DW_VAULT_DIR" ] && echo "denver-workflow: DW_VAULT_DIR='$DW_VAULT_DIR' 폴더 없음 — 규약 경로 시도" >&2
  if [ -d "$CONV" ]; then
    VAULT="$CONV"
  else
    echo "denver-workflow: vault 없음 ($CONV). 'make scaffold-vault' 로 생성하거나 DW_VAULT_DIR 설정." >&2
    exit 1
  fi
fi
VENV="$ROOT/.venv"
