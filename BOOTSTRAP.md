...
```
        ...
        └──────────────┬───────────────────────────────┬───────────────┘     read/write(draft)
            ...
            프로젝트 스킬·검사·훅·에이전트       (8 도구; LIVE stable 직행·OBEY draft)
                       │                               │
                       ▼                               ▼
                 Travel-One / Balipick 세션  ·  Claude Code (구독)
```
...
8. **사용자/테넌트 데이터 격리는 1급 규칙** — 검증자(`enforced-by: security-qa`)를 붙인다.
   (구현은 프로젝트마다 다르다. Balipick 은 RLS 가 아니라 애플리케이션 수준 토큰 스코핑 — [[0008-app-level-scoping-over-rls]].)
...
- **확장**: 실제 프로젝트(Travel-One PHP · Balipick Flutter) 분석 → 규칙·검증자·결정론 린터.
...
- **MCP 선언 일원화(1.3.3)**: MCP 를 `plugin.json` 의 `mcpServers` 에 단일 선언 — 플러그인이
