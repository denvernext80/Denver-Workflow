...
1. **조사**: 관련 코드·스키마·계약·메모리 함정 실측. 신규 개념은 `grep -rn` 으로 기존 모델 확인 의무.
   **API 를 건드리면 vault `API 명세 — 전체 인덱스` 를 먼저 읽고**(없으면 api-spec-bootstrap 절차)
   해당 도메인 상세 노트 하나를 펼친다 — 이미 있는 엔드포인트를 다시 만들거나 기존 응답 규약을
   어기는 사고를 여기서 막는다. 읽기 범위는 [[api-spec-first]].
   **앱 스케줄러·배치 잡을 건드리면** 같은 방식으로 `배치·크론 명세 — 전체 인덱스` 를 먼저 읽는다
   ([[batch-cron-spec-first]]).
...
10. **마감**: 계약 sign-off → **API 명세 갱신([[api-spec-update]]: 도메인 노트 → 인덱스 카운트 →
    변경 이력 prepend)** → 워크트리/브랜치 정리 → 비자명한 학습만 메모리 기록.
    API 표면을 바꿨는데 명세 갱신과 이력 항목이 없으면 완료가 아니다. 정기 실행(앱 스케줄러·
    배치 잡)을 만들거나 바꿨으면 [[batch-cron-spec-update]] 도 같이 돌린다.
