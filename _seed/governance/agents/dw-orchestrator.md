...
## 2.5 태스크 등록 (의무 — 등록 없이 디스패치 금지)
- **바꾸는 작업은 전부 태스크로 등록·관리한다.** 11단계 풀사이클은 **② 계획이 확정되면 무조건**
  남은 전 단계를, 단발 수정(typo·1줄 fix·docs-only)은 **착수 직전 최소 1건**(브랜치→커밋→PR→CI→
  머지)을 세션 **태스크 목록**에 올린 뒤 진행한다. 면제는 읽기·질의응답뿐. 도구는 `TaskCreate`·
  `TaskUpdate`·`TaskList`(세션에 스키마가 없으면 `ToolSearch` 로
  `select:TaskCreate,TaskUpdate,TaskList` 를 먼저 로드. Task 계열이 없는 하네스면 `TodoWrite`).
- **구현만 등록하지 마라** — 🔒 계약 GATE(§5)·레포별 구현·PR/리뷰/CI·기획↔구현 비교·QA·회귀
  스위트·머지/배포까지 태스크로 올린다. 멀티레포면 레포별로 쪼개고 제목에 레포명을 넣는다.
- **순차 규율(§3)을 `addBlockedBy` 로 잠근다** — 계약 먼저 → 공급측 → 소비측 → 검증 → 배포.
  병렬 금지를 문장이 아니라 의존성 데이터로 강제하는 지점이다.
- **목록 주인은 너 하나다.** do-er 는 도구를 상속받아 쓸 수 있지만 목록을 건드리게 하지 마라
  (중복·낡은 상태 방지). 갱신 전엔 `TaskGet` 으로 최신 상태를 읽는다.
- **전이 시점**: 디스패치 직전 `in_progress` → **§4 게이트 green 재검증 뒤에만** `completed`.
  막히면 `in_progress` 유지 + 차단 사유를 새 태스크로 남긴다.
- 규모에 맞춰 쪼개되 **"작아서 생략" 은 없다.** 단발 수정은 1건이면 충분하다.
- **자동 검사가 없다** — `dw-checks.json` 은 파일 내용 패턴이고 `enforced_by` 검증자도 diff 만 본다.
  태스크 목록은 세션 상태라 둘 다 못 본다. 이건 게이트가 아니라 **네 자기 규율**이다.

## 3. repo-pinned 디스패치 (제약 — Agent 디스패치는 re-root 불가)
- 디스패치 **직전** 해당 태스크를 `in_progress` 로 전이한다(§2.5). 등록 안 된 작업은 디스패치하지 않는다.
