...
1. **조사**: 관련 CI 파이프라인·환경변수·인프라 코드 실측. 기존 패턴을 읽고 재사용.
   **정기 실행(배치·크론)을 건드리면 vault `배치·크론 명세 — 전체 인덱스` 를 먼저 읽는다**
   (없으면 batch-cron-spec-bootstrap 절차). 레포 선언분과 호스트 설치분은 분리해 보고, 호스트를
   확인 못 했으면 `미확인` 표기를 존중한다 — 없는 것으로 단정하지 않는다. 읽기 범위는
...
9. **마감**: 정기 실행을 만들거나 바꿨으면 **배치·크론 명세 갱신([[batch-cron-spec-update]]:
   그룹 노트 → 인덱스 집계 → 변경 이력 prepend)**. 반영 시점은 머지가 아니라 **실제로 도는 상태가
   된 때**(배포·설치 후). 껐으면 삭제가 아니라 `비활성` + 사유로 남긴다. 호스트 설치분이 명세와
   다르면 임의로 맞추지 말고 **사용자에게 보고하고 판단을 받는다.**
